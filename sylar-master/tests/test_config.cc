...
void print_yaml(const YAML::Node& node, int level) {                    //node是个map
    if(node.IsScalar()) {                                               //标量
...
public:
    ...
    bool operator==(const Person& oth) const {                  //==操作符的重载
...
#define XX_PM(g_var, prefix) \
    ...
    g_person->addListener([](const Person& old_value, const Person& new_value){                         //测试变更事件
